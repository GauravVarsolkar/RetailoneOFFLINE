...
object OfflineCartCalculator {
    ...
    fun calculateCartTotals(cartItems: List<StoreProData>): PosAddToCartRes {
        ...
        cartItems.forEach { item ->
            ...
            // ✅ Get tax rate (default to 18 if null, but check for 0 explicitly)
            val taxRate = item.tax ?: 18
            ...
            Log.d(TAG, "========================================")
            Log.d(TAG, "Product: ${item.product_name}, Tax Rate: $taxRate%")
            ...
            item.batch.forEach { batch ->
                ...
                if (qty > 0) {
                    ...
                    val unitPriceExclTax = if (taxRate == 0) {
                        // For 0% tax: price is already tax-exclusive
                        round2(priceInclusiveTax)
                    } else {
                        // For taxed products: remove tax from price
                        val taxMultiplier = 1.0 + (taxRate / 100.0)
                        round2(priceInclusiveTax / taxMultiplier)
                    }

                    // ✅ STEP 2: Subtotal BEFORE discount = unit_price × qty
                    val subtotalBeforeDiscount = round2(unitPriceExclTax * qty)

                    // ✅ STEP 3: Apply discount
                    val itemTotal = round2(subtotalBeforeDiscount - batchDiscount)

                    // ✅ STEP 4: Calculate tax AFTER discount
                    val taxAmount = if (taxRate == 0) {
                        0.0
                    } else {
                        BigDecimal.valueOf(itemTotal * (taxRate / 100.0))
                    ...
                    Log.d(TAG, "    Subtotal before discount: $subtotalBeforeDiscount")
                    Log.d(TAG, "    Discount: $batchDiscount")
                    Log.d(TAG, "    Item Total (after discount): $itemTotal")
                    Log.d(TAG, "    Tax ($taxRate%): $taxAmount")
            ...
            if (productSubTotal > 0) {
                ...
                val productTotalWithTax = productSubTotal + productTax

                cartResDataList.add(
                    AddToCartResData(
                        distribution_pack_id = item.distribution_pack_id,
                        distribution_pack = distPack,
                        price_without_discount = productSubTotal + productDiscount,  // ✅ Price before discount
                        product_id = item.product_id,
                        product_name = item.product_name,
                        batch = batchCartItems,
                        stock_id = item.store_id,
                        total = productTotalWithTax,
                        tax_amount = productTax.toString(),
                        tax = taxRate,
        ...
        val grandTotal = subTotalRounded + taxRounded
        ...
        return PosAddToCartRes(
            ...
            sub_total_after_discount = subTotalRounded,
            tax = "@${maxTaxRate}%",
